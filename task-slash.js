ALL_TASKS["slash"] = {
  ...
  onChangeLabel(hand, newLabel, oldLabel) {
    ...
    // Play a sound
    if (newLabel === "🗡") this.swordHitSound.play();
    else this.bubblePopSound.play();
    ...
    // Random color?
    if (newLabel === "🗡") this.color = [360, 100, 50];
    else this.color = [100, 100, 50];
    // this.color = [360*Math.random(), 100, 50]
    // this.color = [360 , 100, 50];
    this.points++;
  },
    ...
    hands.forEach((hand) => {
      ...
      if (hand.isActive) {
        ...
        if (hand.prediction) {
          ...
          hand.points.forEach((pt) => p.text(pred.label,...pt));
        }
      }
    });
